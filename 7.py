...
def ritm_puha(list):
    count = 0
    for vowel in list:
        if vowel in ["а", "я", "у", "ю", "о", "е", "ё", "э", "и", "ы"]:
            count += 1
    return count

result = set((map(ritm_puha, input("enter: ").split())))
print("Пам парам" if len(result) > 1 or list(result)[0] == 0 else "Парам пам-пам")
